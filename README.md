...
- 优化了 DiskMirror 的核心组件版本升级到 1.4.6
- 修复了无法使用 JedisMapper 存储配置数据的问题

### 2025-02-04 版本发布

- 集成了 DiskMirror 核心组件版本升级到 1.4.4

### 2025-01-04 版本发布

- 优化 log 框架的 logger 标签的装载，本次更新后，war 的日志 将不会与 web 容器混合！

### 2024-12-05 版本发布

- 修复 download 函数中 如果userId 是负数会匹配不上的问题
- 对 log 框架添加了 `<scope>provided</scope>` 标签，避免出现日志冲突！
- 对一些 servlet 库添加了 `<scope>provided</scope>` 标签，避免出现冲突！
